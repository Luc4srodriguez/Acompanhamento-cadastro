    ...
    try {
      ...
      function parseIndividuosComIne(data) {
        ...
        for (const r of data || []) {
          ...
          // 1) tenta pelos cabeçalhos configurados em g.HEADERS.ine
...
function detectarParametrosPorStringsCandidatas() {
  ...
  function calcularVinculos(individuos) {
    ...
    for (const row of individuos) {
      const estabelecimento = row.estabelecimento || row['UNIDADE DE SAÚDE'] || row['unidade de saúde'] || row['unidade'] || 'Sem estabelecimento';
      const acs = row.acs || row['PROFISSIONAL CADASTRANTE'] || row['profissional cadastrante'] || row['ACS'] || '';
      // Tenta ler o INE diretamente dos campos conhecidos da linha (inclusive coluna R da planilha)
      const ineRawFromData =
        row.ine ??
        row.INE ??
        row.Ine ??
        row.iNE ??
        ...
        row['iNE'] ??
        null;
      const ineFromData = ineRawFromData ? ineRawFromData.toString().trim() : '';
      const ineFromText = extrairINE(acs || estabelecimento);
      const ine = ineFromData || ineFromText || '';

      const equipeId = ine || 'SEM_INE';
    ...
    for (const [, info] of porEquipe.entries()) {
      ...
      vinculos.push({
        ...
        acs: info.acs,
        equipeOriginal: info.ine || 'SEM_INE',
        ine: info.ine || '',
        total,
        parametroEquipe,
        limiteEquipe,
  ...
  // Modal para exibir profissionais (ACS) de uma equipe específica (por INE)
  function criarModalProfissionais() {
    let modal = document.getElementById('modal-profissionais');

    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'modal-profissionais';
      ...
      content.style.boxShadow = '0 20px 45px rgba(15,23,42,0.35)';
      ...
      const title = document.createElement('h3');
      title.className = 'modal-profissionais-title';
      title.textContent = 'Profissionais da equipe';
      ...
      const body = document.createElement('div');
      body.className = 'modal-profissionais-body';
  ...
  function mostrarProfissionaisDaEquipe(unidade, equipeInfo, individuosBase) {
    const base = Array.isArray(individuosBase) && individuosBase.length
      ? individuosBase
      : (Array.isArray(g.dadosIndividuos) ? g.dadosIndividuos : []);

    const unidadeNorm = String(unidade || '').trim().toLowerCase();
    const ineEquipe = String(equipeInfo.ine || '').trim();

    const registrosEquipe = base.filter((ind) => {
      if (!ind) return false;
      const est = String(ind.estabelecimento || '').trim().toLowerCase();
      if (est !== unidadeNorm) return false;

      const ineInd = String(ind.ine || '').trim();
      if (ineEquipe) {
        return ineInd === ineEquipe;
      }
      // Equipe "SEM_INE": considera registros sem INE
      return !ineInd;
    });

    const mapaProf = new Map();
    registrosEquipe.forEach((ind) => {
      const nomeProf = (ind.acs || 'Sem profissional').trim() || 'Sem profissional';
      const atual = mapaProf.get(nomeProf) || { nome: nomeProf, total: 0 };
      atual.total += 1;
      mapaProf.set(nomeProf, atual);
    });

    const modal = criarModalProfissionais();
    const title = modal.querySelector('.modal-profissionais-title');
    const body  = modal.querySelector('.modal-profissionais-body');

    const ineLabel = ineEquipe || 'SEM_INE';

    if (title) {
      title.textContent = `Profissionais da equipe ${ineLabel} - ${unidade}`;
    }

    if (!registrosEquipe.length) {
      body.innerHTML = '<p style="margin:0.5rem 0;">Nenhum profissional encontrado para esta equipe.</p>';
    } else {
      const profissionaisOrdenados = Array.from(mapaProf.values())
      ...
      table.innerHTML = `
        <thead>
          <tr>
            <th style="text-align:left; padding:8px; border-bottom:1px solid #e5e7eb;">Profissional (ACS)</th>
            <th style="text-align:right; padding:8px; border-bottom:1px solid #e5e7eb;">Nº de pessoas vinculadas</th>
          </tr>
        </thead>
        <tbody>
          ${profissionaisOrdenados.map(p => `
            <tr>
              <td style="padding:6px 8px; border-bottom:1px solid #f3f4f6;">${p.nome}</td>
              <td style="padding:6px 8px; border-bottom:1px solid #f3f4f6; text-align:right;">${p.total}</td>
            </tr>
          `).join('')}
        </tbody>
      `;
  ...
  function mostrarEquipesDaUnidade(unidadeResumo, baseIndividuos) {
    ...
    const linhas = equipes
      .map((e) => `
        <tr>
          <td>${(e.ine || e.equipeOriginal || 'SEM_INE')}</td>
    ...
    const labels = equipes.map((e) => (e.ine || e.equipeOriginal || 'SEM_INE').toString());
    ...
    chartVinculosEquipes = new Chart(canvas.getContext('2d'), {
      ...
      data: {
        labels,
        datasets: [{
          label: 'Pessoas vinculadas (por equipe/INE)',
          data,
          borderWidth: 1,
        }]
      },
      options: {
          ...
          tooltip: {
            callbacks: {
              label: function (ctx) {
                ...
                const e = equipes[idx];
                const ineLabel = (e.ine || e.equipeOriginal || 'SEM_INE').toString();
                return `${ineLabel} - ${e.total.toLocaleString('pt-BR')} pessoas`;
              }
            }
          }
        },
        ...
        onClick: (evt, elements) => {
          ...
          const equipeSelecionada = equipes[idx];
          if (!equipeSelecionada) return;

          const base = Array.isArray(baseIndividuos) && baseIndividuos.length
            ? baseIndividuos
            : (Array.isArray(g.dadosIndividuos) ? g.dadosIndividuos : []);

          mostrarProfissionaisDaEquipe(unidadeResumo.unidade, equipeSelecionada, base);
        }
      }
    });
  }
    ...
    chartVinculos = new Chart(ctx, {
      ...
      options: {
        ...
        onClick: (evt, elements) => {
          ...
          if (unidadeResumo) {
            mostrarEquipesDaUnidade(unidadeResumo, base);
          }
        }
      }
    });
