...
// ATUALIZADO: COM BOTÃO GESTOR + CORREÇÕES DE HISTÓRICO E VÍNCULOS
// ========================================================================
...
async function generateShareableLinkProf(professionalName) {
  if (!professionalName) { alert('Por favor, selecione um profissional primeiro.'); return null; }
  if (!window.dadosIndividuos || !window.dadosDomicilios) { alert('Os dados do painel ainda não foram carregados.'); return null; }
...
// --- Geração de Link GESTOR (NOVA FUNÇÃO) ---
async function generateShareableLinkGestor(unitName) {
  if (!unitName) { alert('Selecione uma unidade no filtro acima.'); return null; }
  
  const ind = (window.dadosIndividuos || []).filter(p => p.estabelecimento === unitName);
  const dom = (window.dadosDomicilios || []).filter(d => d.estabelecimento === unitName);

  if (ind.length === 0 && dom.length === 0) { alert('Nenhum dado encontrado para esta unidade.'); return null; }
  ...
  const combinedPayload = { 
      dados: [...normalizedInd, ...normalizedDom],
      tipo: 'gestor',
      unidade: unitName,
      municipio: window.currentMunicipality
  };

  const sessionId = await sendPayloadToServer(combinedPayload);
  return sessionId ? `gestor.html?id=${sessionId}` : null;
}
...
// Definimos no window para acesso pelo vinculos.js
...
function atualizarFiltros(dados) {
  ...
  const btnGestor = document.getElementById('btnGenerateLinkGestor');

  // Salva o valor atual caso esteja selecionado
  const currentU = selU ? selU.value : '';
  const currentP = selP ? selP.value : '';

  if(selU) {
    selU.innerHTML = `<option value="">Todas as unidades</option>` + Array.from(unidades).sort((a, b) => a.localeCompare(b, 'pt-BR')).map(u => `<option value="${u}">${u}</option>`).join('');
    selU.value = currentU; // Restaura seleção
    
    // Atualiza estado do botão gestor na carga inicial
    if(btnGestor) btnGestor.disabled = !currentU;
  }
  
  if(selP) {
    selP.innerHTML = `<option value="">Todos os profissionais</option>` + Array.from(profissionais).sort((a, b) => a.localeCompare(b, 'pt-BR')).map(p => `<option value="${p}">${p}</option>`).join('');
    selP.value = currentP;
  }
}
...
function sortData(type) {
  ...
  const data = (type === 'individuos') ? window.dadosIndividuos : window.dadosDomicilios;
...
// === FIX: GERAR HISTÓRICO COM BUSCA INTELIGENTE DE CHAVE ===
function gerarHistoricoChart() {
  ...
  let historico = [];
  let keys = Object.keys(localStorage).filter(k => k.startsWith('cadastroHistorico'));
  let chosenKey = 'cadastroHistorico'; // default

  // 1. Prioriza chave com município exato
  if (window.currentMunicipality) {
      const candidateKey = `cadastroHistorico_${norm(window.currentMunicipality)}`;
      if (localStorage.getItem(candidateKey)) {
          chosenKey = candidateKey;
      }
  } 
  
  // 2. Se não achou exata (ou não tem município), tenta achar qualquer chave específica disponível
  if (chosenKey === 'cadastroHistorico' && keys.length > 0) {
      const specific = keys.find(k => k !== 'cadastroHistorico');
      if (specific) chosenKey = specific;
  }
  ...
  if (historico.length === 0) {
    messageEl.textContent = "Nenhum histórico de processamento encontrado para este município.";
...
function switchTab(tabId) {
  ...
  // ADIÇÃO: Suporte para aba Vínculos chamando a função gerarDashboard (que será sobrescrita pelo vinculos.js)
  if (tabId === 'dashboard') { window.gerarDashboard(window.dadosIndividuos.filter(filterFn), window.dadosDomicilios.filter(filterFn)); }
  else if (tabId === 'vinculos') { 
      // Chama gerarDashboard para ativar o hook do vinculos.js e atualizar o painel de vínculos
      window.gerarDashboard(window.dadosIndividuos.filter(filterFn), window.dadosDomicilios.filter(filterFn)); 
  }
  else if (tabId === 'individuos') { popularTabelaIndividuos(window.dadosIndividuos.filter(filterFn)); }
  else if (tabId === 'domicilios') { popularTabelaDomicilios(window.dadosDomicilios.filter(filterFn)); }
  else if (tabId === 'relatorio-unidade-ind') { gerarRelatorioPorUnidadeInd(window.dadosIndividuos.filter(filterFn)); }
  else if (tabId === 'relatorio-unidade-dom') { gerarRelatorioPorUnidadeDom(window.dadosDomicilios.filter(filterFn)); }
  else if (tabId === 'relatorio-prof-ind') { gerarRelatorioPorProfissionalInd(window.dadosIndividuos.filter(filterFn)); }
  else if (tabId === 'relatorio-prof-dom') { gerarRelatorioPorProfissionalDom(window.dadosDomicilios.filter(filterFn)); }
...
function aplicarFiltrosEViews() {
  ...
  const filtradosInd = window.dadosIndividuos.filter(filterFn);
  const filtradosDom = window.dadosDomicilios.filter(filterFn);
  
  // ADIÇÃO: Suporte para aba Vínculos nos filtros
  switch (activeTab) {
    case 'dashboard': window.gerarDashboard(filtradosInd, filtradosDom); break; 
    case 'vinculos': window.gerarDashboard(filtradosInd, filtradosDom); break; // Atualiza vínculos
...
function populateProfessionalSelectCoord() {
  ...
  const profissionais = new Set();
  [...window.dadosIndividuos, ...window.dadosDomicilios].forEach(item => {
    if (item.acs) profissionais.add(item.acs);
  });
...
async function loadDataFromLink() {
  ...
  try {
    ...
    // Salva explicitamente no window para acesso global
    window.dadosIndividuos = payload.dados.individuos || [];
    window.dadosDomicilios = payload.dados.domicilios || [];
    
    // Tenta detectar município se disponível no payload ou define padrão
    if (payload.municipio) {
        window.currentMunicipality = payload.municipio;
    }
    ...
    // Dispara evento para avisar scripts externos (como vinculos.js) que os dados chegaram
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const selU = document.getElementById('filterUnidade');
  const selP = document.getElementById('filterProfissional');
  const btnGenLinkGestor = document.getElementById('btnGenerateLinkGestor');

  // Adiciona listener no SELECT de UNIDADE para habilitar/desabilitar o botão Gestor
  if (selU) {
      selU.addEventListener('change', () => {
          aplicarFiltrosEViews();
          if (btnGenLinkGestor) {
              btnGenLinkGestor.disabled = !selU.value;
          }
      });
  }
  ...
  // EVENT LISTENER DO BOTÃO GESTOR
  if (btnGenLinkGestor) {
      const originalBtnHTML = btnGenLinkGestor.innerHTML;
      btnGenLinkGestor.addEventListener('click', async () => {
          const unidadeSelecionada = document.getElementById('filterUnidade').value;
          if (!unidadeSelecionada) { 
              alert('Por favor, selecione uma unidade no filtro principal.'); 
              return; 
          }
          
          btnGenLinkGestor.disabled = true; 
          btnGenLinkGestor.textContent = 'Gerando Link...';
          
          const relativeUrl = await generateShareableLinkGestor(unidadeSelecionada);
          if (relativeUrl) { 
              copyToClipboard(relativeUrl); 
          }
          
          btnGenLinkGestor.disabled = false; 
          btnGenLinkGestor.innerHTML = originalBtnHTML;
      });
  }
